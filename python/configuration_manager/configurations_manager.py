...
class ConfigurationsManager:
    ...
    def make_directory(self, directory, path_to_directory=None):
        """Wrapper for making directories"""
        if path_to_directory is None:
            path_to_directory = self.get_directory(DefaultDirectories.OUTPUT)
    ...
    def load_log_configurations(self, name,
        ...
        Return a logger
        """
        if directory and path_to_directory is not None:
            # Case: make directory at the target location for the logs
            target_directory = self.make_directory(
                                        directory, path_to_directory)
        else:
